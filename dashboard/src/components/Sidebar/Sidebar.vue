  ...
  <aside
    ...
    <div class="mb-8 px-3">
      <div class="flex items-center" :class="{ 'justify-center': !isActive }">
        ...
        <span v-if="isActive" class="ml-3 font-bold text-lg whitespace-nowrap clear-font">NanoBot</span>
      </div>
    </div>

    <!-- Navigation -->
    <nav class="flex-1 w-full space-y-2">
      <button v-for="item in menuItems" :key="item.id" @click="navigate(item.id)"
    ...
    <!-- Toggle Button -->
    <button @click="$emit('toggle')" class="mt-4 p-2 hover:bg-gray-700 rounded-lg transition-colors">
