...
const interruptable = ref(false) // Track if can interrupt speaking state
...
const connectWebSocket = () => {
  ...
  ws.onopen = () => {
    ...
    // Initialize connection: send /inspect and /history commands (hidden from chat)
...
// Handle toggle speak from button click
...
const handleSendStatus = () => {
  ...
  // Send /inspect command via Chat component's sendMessage
  if (chatComponentRef.value && chatComponentRef.value.sendMessage) {
    chatComponentRef.value.sendMessage('/inspect', true)
  }
}
