...
When generating HTML files for Three.js demos, save them to:
- Directory: `~/.nanobot/media/threejs/`
- Use absolute paths when referencing the saved file

Example:
```javascript
// Save to ~/.nanobot/media/threejs/demo.html
const outputPath = Path.home() / '.nanobot' / 'media' / 'threejs' / 'demo.html';
```
