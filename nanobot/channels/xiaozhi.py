...
class XiaoZhiChannel(BaseChannel):
    ...
    def default_config(cls) -> dict[str, Any]:
        return XiaoZhiConfig().model_dump(by_alias=True)

    def __init__(self, config: XiaoZhiConfig, bus: MessageBus):
        """
        Initialize the XiaoZhi channel with the given configuration and message bus.
        ...
        if isinstance(config, dict):
            config = XiaoZhiConfig.model_validate(config)
